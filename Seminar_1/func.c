...
void read_units(FILE *file, Node **head) {
    while (1) {
        Sale_unit unit;
        unit.date = (char *)malloc(MAX_LEN * sizeof(char));
        unit.product_name = (char *)malloc(MAX_LEN * sizeof(char));
        unit.product_category = (char *)malloc(MAX_LEN * sizeof(char));
        unit.product_subcategory = (char *)malloc(MAX_LEN * sizeof(char));
        unit.country = (char *)malloc(MAX_LEN * sizeof(char));
        unit.city = (char *)malloc(MAX_LEN * sizeof(char));
        ...
        if (fscanf(file, "%s %d %s %s %s %f %d %s %s",
            ...
            free(unit.product_category);
            free(unit.product_subcategory);
            free(unit.country);
            free(unit.city);
            break;
        }
...
void free_list(Node *head) {
    ...
    while (head != NULL) {
        ...
        head = head->next;
        free(temp->unit.date);
        free(temp->unit.product_name);
        free(temp->unit.product_category);
        free(temp->unit.product_subcategory);
        free(temp->unit.country);
        free(temp->unit.city);
        free(temp);
    }
}